# README: Repository Architecture Overview

## Purpose
This README explains how the repo is split between backend data modeling and frontend visualization. It is the root navigation file for the detailed per-file READMEs below.

## Core System Flow
1. Raw class schedule data lives in `backend/classes/parsed_sections.json`.
2. Dining wait-time CSVs live under `backend/wait_times/*.csv`.
3. Two backend density models convert those datasets into people-density fields:
   - `backend/classes/density_field.py`
   - `backend/wait_times/density_field.py`
4. Frontend map logic consumes timeline snapshots and renders heatmaps + building overlays:
   - `frontend/components/map/campus-map.tsx`
   - `frontend/components/map/heatmap-layer.tsx`
   - `frontend/lib/map/mock-data.ts`
5. Optional backend artifact generation (GIF/timeline exports) uses:
   - `backend/create_combined_heatmap_gif.py`

## High-Importance File READMEs
- `backend.classes.density_field.readme.md`
- `backend.wait_times.density_field.readme.md`
- `backend.classes.parsed_sections.readme.md`
- `backend.create_combined_heatmap_gif.readme.md`
- `frontend.components.map.campus-map.readme.md`
- `frontend.components.map.heatmap-layer.readme.md`
- `frontend.lib.map.mock-data.readme.md`
- `frontend.lib.map.buildings.readme.md`
- `frontend.lib.map.config.readme.md`

## What Is Cool About This Repo
- It blends two independent signals (scheduled classes and queue-driven dining activity) into one spatial density visualization.
- The class model includes temporal realism (arrival ramp, post-class linger, decay), so population does not unrealistically drop to zero at boundary times.
- The frontend map stack combines Mapbox basemap + deck.gl aggregation + interaction overlays in a clean separation of concerns.

## Practical Notes
- When editing model behavior, keep unit assumptions explicit: all density outputs are intended to be in estimated people units.
- When heatmaps “look wrong,” verify three layers in order:
  1) point source data,
  2) map layer tuning,
  3) UI time/day synchronization.
