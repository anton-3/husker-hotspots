# README: backend/classes/parsed_sections.json

## Purpose
This JSON file is the core class schedule dataset driving class-derived occupancy modeling.

## What It Contains
- One object per section offering (term/course/section identity).
- Enrollment and capacity information.
- Location fields (`buildingCode`, `location`, `room`).
- Geospatial coordinates (`latitude`, `longitude`) when available.
- `classTimes` meeting structures with `days`, `startTime`, `endTime`.

## How It Is Used
- Parsed and indexed by `backend/classes/density_field.py`.
- Meeting times are mapped to weekday/time slots.
- Enrollment/capacity values seed estimated people counts.
- Coordinates anchor Gaussian density sources.

## Data Quality Considerations
- Records with null coordinates cannot contribute to spatial modeling.
- Sections with ambiguous or missing `classTimes` do not contribute to active-slot occupancy.
- Because this dataset is schedule-based, it reflects expected attendance patterns, not real-time presence telemetry.

## Why It Is Cool
It provides enough structure to support both high-level campus heatmaps and detailed building-level explanations while still being plain JSON.
