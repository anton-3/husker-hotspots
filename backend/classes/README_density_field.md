# README: backend/classes/density_field.py

## File Purpose
This file is the class-schedule people-density model. It transforms section-level schedule records into a continuous spatial field (`people_field`) for a requested weekday and time.

## Why This File Exists
- Class schedules are discrete (start/end times, building points).
- Heatmaps need continuous values over a grid.
- This file bridges that gap while preserving people-unit semantics and realistic transition behavior.

## Primary Responsibilities
1. Load + cache parsed course sections from `backend/classes/parsed_sections.json`.
2. Filter to geocoded meetings that occur on the requested weekday.
3. Estimate attendees per section (`enrolled` preferred, `capacity` fallback).
4. Model time transitions:
   - pre-arrival ramp,
   - in-class full presence,
   - post-class linger/travel decay,
   - time smoothing with small interpolation offsets.
5. Aggregate section attendance by building.
6. Spread building people counts into a Gaussian density field over campus bounds.

## Inputs and Outputs
### Inputs
- `requested_time` (HH:MM, snapped to 15 minutes)
- `weekday` (full weekday name)
- optional map bounds/resolution/spread/attendance parameters
...
- `bounds`, `resolution`
- `lng_axis`, `lat_axis`
- `parse_stats` (coverage insight)
- `active_buildings` (building-level estimated people)
- `people_field` (2D floating grid in estimated people density units)

## Interactions With Other Files
- Reads: `backend/classes/parsed_sections.json`
- Conceptually paired with: `backend/wait_times/density_field.py`
- Consumed indirectly by generation/visualization scripts such as `backend/create_combined_heatmap_gif.py`
- Output semantics are expected by frontend visualization code in `frontend/components/map/heatmap-layer.tsx`

## Modeling Notes and Tradeoffs
- Uses `@lru_cache` to avoid reparsing huge class JSON on repeated calls.
- Uses weekday character mapping (`M`, `T`, `W`, `R`, etc.) from class-time records.
- Presence smoothing is intentionally local (`-7.5, 0, +7.5` min) to reduce visual flicker without over-blurring schedule changes.
- Post-class spread widening models movement after dismissal (not everyone remains fixed at classroom centroid).

## What Is Cool Here
- It is not just “active/not active.” It models class transitions as a physically plausible process.
- It outputs both machine-friendly field data and human-debuggable metadata (`parse_stats`, `active_buildings`).

## Risks When Editing
- Small constant changes can significantly alter perceived crowd behavior.
- Changing unit semantics breaks comparability with wait-time model and frontend thresholds.
- Boundary logic around time snapping can create silent off-by-one-slot artifacts.
