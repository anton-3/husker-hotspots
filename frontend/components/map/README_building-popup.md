# README: frontend/components/map/building-popup.tsx

## Purpose
Renders the right-side building detail panel when a building is selected.

## Responsibilities
- Show current occupancy and capacity context.
- Display 24-hour occupancy trend chart.
- Present source-contribution breakdown for current frame.
- Expose building metadata (hours, description, type).

## Interactions
- Receives selected building from `campus-map.tsx`.
- Uses timeline snapshots from `frontend/lib/map/mock-data.ts`.
- Uses building color/type metadata from `frontend/lib/map/buildings.ts`.
- Pulls source labels/colors from `frontend/lib/map/config.ts`.

## Why It Is Important
This is the main explanatory surface that translates abstract heat colors into actionable building-level interpretation.
