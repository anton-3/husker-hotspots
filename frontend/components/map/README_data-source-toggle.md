# README: frontend/components/map/data-source-toggle.tsx

## Purpose
Provides toggles for source categories (classes, dining, sports, etc.) that influence the effective heatmap intensity.

## Interactions
- Source definitions come from `frontend/lib/map/config.ts`.
- State is managed in `campus-map.tsx` and passed down.
- Affects rendered heatmap via scaling logic in `heatmap-layer.tsx`.

## Notes
- Current behavior scales total weights by fraction of active sources (simple and predictable).
- If future requirements need per-point source attribution, this component can stay unchanged while downstream filtering logic evolves.
