# README: frontend/components/map/heatmap-layer.tsx

## Purpose
Defines all deck.gl layers used by the map, including the primary `HeatmapLayer` and interactive `GeoJsonLayer` for building footprints.

## Responsibilities
- Convert upstream `heatmapPoints` into a styled density layer.
- Apply source-filter scaling (active source ratio).
- Render clickable/hoverable building polygons with selected/hovered visual states.

## File Interactions
- Consumes `HeatmapPoint`/`BuildingOccupancy` from `frontend/lib/map/mock-data.ts`.
- Consumes building metadata/colors from `frontend/lib/map/buildings.ts`.
- Called by `frontend/components/map/campus-map.tsx` and returned layers are rendered through `deck-overlay.tsx`.

## Heatmap Tuning Notes
- `radiusPixels`, `intensity`, `threshold`, and `opacity` jointly control visibility.
- If heatmap appears “missing,” first inspect whether `data` is empty, then tune these params.

## Cool Bits
- Co-locates both passive visualization (heatmap) and interactive geometry (footprints) in one hook.
- Uses update triggers for precise rerender behavior on hover/selection without full layer churn.
