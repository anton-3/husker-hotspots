# README: frontend/components/map/insights-panel.tsx

## Purpose
Displays AI-like insight cards derived from current and near-future occupancy snapshots.

## Interactions
- Consumes `Insight[]` generated in `frontend/lib/map/mock-data.ts`.
- Called by `campus-map.tsx`, and click callbacks route users to selected buildings.

## UX Role
- Converts data into narrative guidance (alerts, predictions, recommendations).
- Acts as a decision-support layer beyond raw heatmap color intensity.
