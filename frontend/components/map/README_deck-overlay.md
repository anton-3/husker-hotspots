# README: frontend/components/map/deck-overlay.tsx

## Purpose
Bridges deck.gl layers into the Mapbox map instance through `react-map-gl` control lifecycle.

## Why This File Exists
Without this adapter, `HeatmapLayer` and `GeoJsonLayer` logic cannot be rendered on top of the Mapbox basemap in an interleaved, z-correct way.

## Interactions
- Receives computed layers from `campus-map.tsx`.
- Uses `MapboxOverlay` from `@deck.gl/mapbox`.

## Cool Aspect
Tiny file, huge impact: one place cleanly handles the map/deck integration seam.
