# README: frontend/components/map/timeline-slider.tsx

## Purpose
Provides user controls for map time navigation: day switching, playback, speed, skip, and continuous index selection.

## Why It Matters
Time index is the primary selector for heatmap snapshots. If this component drifts from data indexing semantics, the map shows wrong frames.

## Interactions
- Receives and emits timeline state managed in `campus-map.tsx`.
- Uses weekday values from `frontend/lib/map/config.ts`.
- Assumes `0..47` half-hour slots generated by `frontend/lib/map/mock-data.ts`.

## Behavioral Contracts
- `onIndexChange` must remain authoritative for timeline frame changes.
- Skip buttons move by 2 slots (1 hour).
- Speed controls currently expose `1x`, `2x`, `4x`.

## Cool Detail
The component is intentionally stateless: all temporal truth lives in parent state, minimizing hidden synchronization bugs.
