# README: frontend/components/map/legend.tsx

## Purpose
Provides visual decoding for heatmap color gradients and occupancy threshold buckets.

## Interactions
- Appears in `campus-map.tsx` as a passive explanatory overlay.
- Should stay semantically aligned with `heatmap-layer.tsx` color ramp and occupancy conventions.

## Why It Matters
Without the legend, users can see intensity but cannot reliably interpret severity ranges.
