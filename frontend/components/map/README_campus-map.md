# README: frontend/components/map/campus-map.tsx

## Purpose
`campus-map.tsx` is the orchestration layer for the entire map experience. It wires time/day state, user controls, map camera, deck.gl overlays, data-source toggles, insights, and building detail UX into a single interactive screen.

## Primary Use Cases
- Render the main map page users interact with.
- Drive heatmap playback over time via timeline controls.
- Allow users to click/hover buildings and inspect occupancy details.
- Coordinate source filters (classes, dining, clubs, etc.) and pass them to rendering layers.

## How It Interacts With Other Files
- Pulls map constants and bounds from `frontend/lib/map/config.ts`.
- Pulls campus geometry/metadata from `frontend/lib/map/buildings.ts`.
- Pulls time-snapshot data and insights from `frontend/lib/map/mock-data.ts`.
- Delegates rendering layers to `frontend/components/map/heatmap-layer.tsx`.
- Bridges deck.gl into Mapbox via `frontend/components/map/deck-overlay.tsx`.
- Uses child UI controls/components:
  - `timeline-slider.tsx`
  - `data-source-toggle.tsx`
  - `insights-panel.tsx`
  - `building-popup.tsx`
  - `legend.tsx`

## Key State Domains
- **Map view state:** camera center/zoom/pitch/bearing.
- **Temporal state:** weekday + time index + play/pause + playback speed.
- **Selection state:** hovered building, selected building popup.
- **Filter state:** active data sources.
- **Map lifecycle:** waits for map load before mounting deck overlay.

## Why This File Is Important
This is the highest-leverage frontend map file. If heatmaps, interactions, or timeline syncing fail, the cause is usually in this orchestration layer or one of its immediate dependencies.

## Cool Implementation Details
- Adds a campus-only 3D building extrusion layer using `within` bounds filtering.
- Keeps map interaction smooth with memoization and callback boundaries.
- Uses map fly-to transitions for polished focus changes on building selection.

## Editing Guidance
- Preserve parent->child prop contracts when changing controls.
- Keep data time index (`0..47`) consistent with timeline and mock-data slot generation.
- Validate `heatmapPoints` source whenever changing timeline/data plumbing.
