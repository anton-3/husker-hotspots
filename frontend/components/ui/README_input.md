# README: frontend/components/ui/input.tsx

## Purpose
`input.tsx` defines the default text-entry primitive used across forms and interactive controls.

## Role in the System
- Standardizes text input styling and behavior.
- Encodes baseline accessibility and validation affordances.
- Prevents style drift between disparate form surfaces.

## Technical Responsibilities
- Render a native `<input>` with project-standard class composition.
- Preserve all native input props while layering shared UI tokens.
- Support file-input pseudo-element styling hooks.
- Apply focus-visible and `aria-invalid` feedback styles.

## Interactions With Other Files
- Uses `cn` from `frontend/lib/utils.ts`.
- Used by forms, filters, and custom control compositions across app/map UIs.
- Works with field wrappers likely defined in sibling `ui/*` components.

## Styling and UX Notes
- Uses tokenized classes (`border-input`, `focus-visible:ring-ring/...`) for design consistency.
- Supports disabled states via pointer and opacity reductions.
- Uses selection color overrides to align highlight behavior with theme palette.

## Accessibility Notes
- Native input semantics are preserved (keyboard input, browser-level behaviors).
- `aria-invalid` styles visually signal invalid fields.
- Focus ring pattern aligns with system-wide visible focus design.

## Cool Implementation Detail
It intentionally stays minimal and purely compositional: no custom state machine, no synthetic behavior, just a clean ergonomic wrapper that remains highly reusable.

## Editing Risks
- Changing base classes affects every downstream form.
- Removing transition/focus classes can degrade perceived responsiveness.
- Over-customizing file pseudo-element classes may break browser consistency.

## Suggested Enhancements (Future)
- Optional size variants via CVA if the app introduces high-density and spacious forms.
- Optional invalid message slot integration through a higher-level field wrapper.
