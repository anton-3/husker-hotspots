# README: frontend/components/ui/dialog.tsx

## Purpose
`dialog.tsx` wraps Radix Dialog primitives into the project’s standard modal API and styling system.

## Why It Matters
Dialog behavior is accessibility-critical and interaction-critical. A robust wrapper ensures consistent modal semantics, keyboard handling, layering, and visual standards.

## Component Surface Exported
- `Dialog`
- `DialogTrigger`
- `DialogPortal`
- `DialogOverlay`
...
- Maintain reliable focus/escape behavior via Radix primitives.
- Ensure overlay + content z-index and animation consistency.
- Provide standard content shell and optional close button.
- Expose composable subcomponents for structured modal layouts.

## Interaction With Other Files
- Uses `cn` from `frontend/lib/utils.ts`.
- Consumed by feature-level screens that require modal flows.
- Visual style should align with shared primitives (buttons, typography, spacing).

## Accessibility Notes
- Uses proper dialog/title/description semantic primitives from Radix.
- Close button includes an SR-only label.
- Focus ring classes are present for keyboard users.

## Behavior Notes
- Overlay and content animations are state-driven via data attributes.
- `showCloseButton` in `DialogContent` gives consumers control over whether explicit chrome close affordance is shown.

## Cool Aspects
- Thin wrapper keeps accessibility heavy-lifting in Radix while project controls styling + API ergonomics.
- Data-slot attributes improve testability and style-targeting clarity.

## Risks When Editing
- Breaking primitive composition can regress focus trapping.
- Changing animation classes can create perceived lag or modal flicker.
- Incorrect z-index edits may hide overlays under other layers.
