# README: frontend/components/ui/select.tsx

## Purpose
`select.tsx` defines the canonical choice-picker primitive (trigger + popup options) using Radix Select primitives with project styling.

## Role in Product UX
Selects are used wherever users need constrained discrete choice input (day, source, options). This component ensures consistent dropdown behavior and style across the app.

## Export Surface
- Structural primitives: `Select`, `SelectGroup`, `SelectValue`
- Trigger/content primitives: `SelectTrigger`, `SelectContent`
- Option primitives: `SelectItem`, `SelectLabel`, `SelectSeparator`
- Scroll controls: `SelectScrollUpButton`, `SelectScrollDownButton`

## Responsibilities
- Normalize trigger look-and-feel and placeholder state styles.
- Render option content with robust keyboard-accessible behavior.
- Display selected indicators on selected items.
- Handle constrained viewport/scroll behavior for large option sets.

## Interactions
- Uses `cn` utility from `frontend/lib/utils.ts`.
- Uses Lucide icons for chevrons/check marks.
- Consumed by feature components that require controlled option selection.

## Implementation Details Worth Noting
- Supports trigger `size` variants (`sm`, `default`).
- Uses Radix portal rendering to avoid clipping issues in nested containers.
- Position mode supports `popper`, with dynamic transform classes per side.

## Accessibility Notes
- Built on Radix Select semantics for keyboard + focus handling.
- Keeps option selection visual and semantic indicators explicit.

## What Is Cool Here
- Gives full-featured select behavior with minimal consumer complexity.
- Encapsulates many tricky dropdown layout concerns (viewport sizing, scroll controls, portal strategy) in one reusable primitive.

## Risks When Editing
- Trigger class edits can break baseline alignment in forms.
- Content positioning edits can produce clipping or off-screen behavior.
- Option padding/indicator edits can hurt scanability and perceived selection state.
