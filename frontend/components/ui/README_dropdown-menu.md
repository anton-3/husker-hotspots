# README: frontend/components/ui/dropdown-menu.tsx

## Purpose
`dropdown-menu.tsx` exposes a complete menu system (root/trigger/content/item/submenu/checkbox/radio variants) built on Radix Dropdown primitives with project-standard styling.

## Why This File Is Important
This file provides one of the most feature-dense interaction primitives in the UI library. Many advanced menus depend on it for keyboard navigation, nested structures, checked/radio state, and action semantics.

## Exported Subcomponents
- Root-level: `DropdownMenu`, `DropdownMenuTrigger`, `DropdownMenuContent`, `DropdownMenuPortal`
- Grouping: `DropdownMenuGroup`, `DropdownMenuLabel`, `DropdownMenuSeparator`, `DropdownMenuShortcut`
- Items: `DropdownMenuItem`, `DropdownMenuCheckboxItem`, `DropdownMenuRadioGroup`, `DropdownMenuRadioItem`
- Nesting: `DropdownMenuSub`, `DropdownMenuSubTrigger`, `DropdownMenuSubContent`

## Core Responsibilities
- Standardize menu spacing, focus states, disabled styles, and variant semantics.
- Support destructive action styling with explicit visual distinction.
- Support item indicators for checkbox/radio selections.
- Support nested submenu interactions with directional icon affordances.

## Interactions With Other Files
- Uses iconography from `lucide-react`.
- Uses `cn` utility from `frontend/lib/utils.ts`.
- Consumed by feature UIs requiring contextual action menus.

## Accessibility and Behavior Notes
- Radix primitives provide robust keyboard navigation and ARIA semantics.
- Focus and open-state visuals are controlled via data attribute classes.
- Disabled states are handled via pointer-events and opacity classes.

## API Design Details
- `DropdownMenuItem` supports `variant` (`default`/`destructive`) and `inset` spacing.
- Specialized item types (`CheckboxItem`, `RadioItem`) preserve built-in Radix state model.
- `DropdownMenuShortcut` provides standardized right-aligned hint text style.

## Cool Aspects
- Highly composable while staying stylistically coherent.
- Submenu support is robust with minimal consumer complexity.
- Offers rich interaction surface without requiring consumers to hand-roll accessibility logic.

## Risks When Editing
- Minor class changes can reduce contrast or break discoverability in dense menus.
- Altering indicator positioning can make checked/radio states ambiguous.
- Inconsistent submenu animation/timing can produce jarring UX.
