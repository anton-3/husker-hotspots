# README: frontend/lib/map/config.ts

## Purpose
Defines global map constants and shared enums/types for the map feature.

## Responsibilities
- Camera defaults and zoom/pitch/bearing constraints.
- Campus bounding box used for map limits and filtering.
- Heatmap color thresholds and occupancy semantic thresholds.
- Canonical source catalog (`DATA_SOURCES`) and weekday catalog (`DAYS_OF_WEEK`).

## Interactions
- Used by `campus-map.tsx` for map setup and temporal controls.
- Used by `data-source-toggle.tsx` for rendering and toggling categories.
- Used by `mock-data.ts` and `building-popup.tsx` for source typing/labels.

## Why It Is Important
This file encodes cross-component vocabulary. It keeps UI and modeling layers aligned by providing one shared definition for what a source/day/threshold means.
