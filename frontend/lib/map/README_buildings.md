# README: frontend/lib/map/buildings.ts

## Purpose
Central static registry of campus buildings used by the visualization system.

## Data Included
- Unique IDs and display names.
- Building type classification.
- Coordinates and approximate footprint polygons.
- Capacity, hours, and descriptive metadata.
- Type-to-color map for consistent styling.

## Interactions
- Read by `mock-data.ts` for occupancy simulation context.
- Read by `heatmap-layer.tsx` for footprint rendering.
- Read by `campus-map.tsx` and `building-popup.tsx` for selection and details.

## Why It Is Important
This is the map’s spatial source of truth. If IDs, coordinates, or polygons are wrong, rendering and analytics interpretation both degrade.
