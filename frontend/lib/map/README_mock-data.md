# README: frontend/lib/map/mock-data.ts

## Purpose
This file is the synthetic timeline engine for the frontend map. It generates per-slot building occupancy, point clouds for heatmaps, and insight cards.

## Core Responsibilities
- Define time-slot schema (`0..47`, half-hour increments).
- Define occupancy source patterns by building type.
- Compute per-building occupancy %, occupant counts, and source decomposition.
- Generate heatmap points around building centroids.
- Produce insights (alerts, predictions, recommendations).

## Interactions
- Uses building metadata from `frontend/lib/map/buildings.ts`.
- Uses data-source taxonomy (`DataSourceId`, weekdays) from `frontend/lib/map/config.ts`.
- Consumed heavily by `frontend/components/map/campus-map.tsx` and child components.

## Why It Is Important
Even as backend-driven data matures, this file defines the shape of map timeline data contracts and the UX expectations around time-playback behavior.

## Cool Aspects
- Uses Gaussian time profiles for realistic diurnal patterns.
- Produces both visualization primitives (heatmap points) and semantic narratives (insights).

## Risk Notes
- Contract changes here ripple into nearly every map component.
- Any change to `TimeSlot` indexing must stay consistent with `timeline-slider.tsx` bounds.
